...
class BinanceRest {
    ...
    constructor({ key, secret, recvWindow, timeout, disableBeautification }) {
        ...
        this._baseUrl3 = `https://www.binance.com/api/v3/`;
    }
    ...
    _makeRequest(query, callback, route, security, method) {
        ...
        const options = {
            ...
            url3: `${this._baseUrl3}${route}`,
            timeout: this.timeout
        }
        if (queryString) {
            options.url3 += '?' + queryString;
        }
        if (security === 'SIGNED') {
            if (options.url3.substr(options.url3.length - 1) !== '?') {
                options.url3 += '&';
            }
            options.url3 += `signature=${this._sign(queryString)}`;
        }
    ...
    allTickers(callback) {
        return this._makeRequest({}, callback, 'ticker/allPrices');
    }
